...
- `summary` (when `includeSummary=true`): aggregates for current filter — at minimum `totalCalls`, `avgResponseTimeMs`, `avgSessionDurationMs`, `bellCount`, `videoCount`, `telegramAcceptCount`, `webAcceptCount`
- `charts` (when `includeCharts=true`): chart series for the active filter window (see REQ-CALL-020)
...
- **AND** `acceptedChannel` SHALL be `telegram`
...
- **THEN** API MUST return 400 (documented in runbook)

...
export function JoinVideoClient() {
  ...
  const exchangeToken = useCallback(async (token: string) => {
    ...
    try {
      const res = await fetch("/api/staff/telegram/video-join", {
        method: "POST",
      ...
      setState({
        ...
        token: data.token,
        callId: data.callId,
        listenConfig: data.listenConfig,
      });
    } catch {
      setState({
        phase: "error",
  ...
  useEffect(() => {
    if (!joinToken) {
      setState({
        phase: "error",
        message: "Enlace inválido. Abra el enlace desde la notificación de Telegram.",
      });
      return;
    }
    ...
    return (
      <main className="flex min-h-dvh flex-col items-center justify-center bg-slate-950 px-6 text-center text-white">
        <div className="max-w-sm rounded-2xl border border-slate-800 bg-slate-900 p-6">
          ...
          <h1 className="mt-4 text-xl font-semibold">No se pudo unir</h1>
          <p className="mt-3 text-sm text-slate-300">{state.message}</p>
          <p className="mt-4 text-xs text-slate-500">
            También puede atender desde el dashboard del hospital.
          </p>
        </div>
      </main>
    );
  ...
  return (
    <VideoCallSession
      ...
      role="staff"
      token={state.token}
      listenConfig={state.listenConfig}
      autoStart
      fullscreen
      onEnded={() => {
